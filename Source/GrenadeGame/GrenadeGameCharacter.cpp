...
AGrenadeGameCharacter::AGrenadeGameCharacter()
{
	...
	NewnumGrenades = FString::Printf(TEXT("%d"), numGrenades);
	TextComponent->SetText(FText::FromString(NewnumGrenades));
...
void AGrenadeGameCharacter::SetupPlayerInputComponent(class UInputComponent* PlayerInputComponent)
{
	...
	//Input for G key
	PlayerInputComponent->BindAction("ThrowGrenade", IE_Pressed, this, &AGrenadeGameCharacter::OnThrowGrenade);
}

//Function to replicate the number of grenades that each character has via server
void AGrenadeGameCharacter::GetLifetimeReplicatedProps(TArray<FLifetimeProperty> & OutLifetimeProps) const 
{
	Super::GetLifetimeReplicatedProps(OutLifetimeProps);

	DOREPLIFETIME(AGrenadeGameCharacter, numGrenades);
}
...
void AGrenadeGameCharacter::OnThrowGrenade()
{
	if (numGrenades > 0) 
	{
		ServerThrowGrenade();
...
void AGrenadeGameCharacter::NetMulticastNum_Implementation() {
	numGrenades--;
	FString NewnumGrenades;
	NewnumGrenades = FString::Printf(TEXT("%d%"), numGrenades);
	TextComponent->SetText(FText::FromString(NewnumGrenades));
